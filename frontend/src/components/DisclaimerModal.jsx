...
export default function DisclaimerModal({ isOpen, onClose, language, setLanguage }) {
  ...
  const handleLanguageSelect = (langCode) => {
    ...
    try {
      localStorage.setItem('detexso_language', langCode);
    } catch (e) {
  ...
  const handleAccept = () => {
    try {
      ...
      localStorage.setItem('detexso_language', language);
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 rounded-3xl max-w-lg w-full shadow-2xl border border-slate-200/90 dark:border-slate-800 relative my-4 sm:my-6 text-left transition-colors duration-300 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-5">
          ...
          <div className="space-y-3 text-xs sm:text-sm leading-relaxed max-h-[48vh] sm:max-h-[46vh] overflow-y-auto pr-1 custom-scrollbar">
            ...
            <div className="p-3.5 sm:p-4 bg-blue-50/90 dark:bg-blue-950/50 border border-blue-200/90 dark:border-blue-900/60 rounded-2xl space-y-1">
              ...
              <p className="text-slate-700 dark:text-slate-300 leading-relaxed text-xs sm:text-sm">
                {language === 'hi'
                  ? 'DeTexSO एक AI-संचालित शैक्षिक और जागरूकता उपकरण है। AI द्वारा दिए गए परिणाम, जोखिम स्कोर और विश्लेषण केवल संभावित अनुमान हैं। इन्हें अंतिम कानूनी, वित्तीय या साइबर सुरक्षा गारंटी न माना जाए।'
                  : language === 'gu'
                  ? 'DeTexSO એ AI-સંચાલિત શૈક્ષણિક અને જાગૃતિનું સાધન છે. AI દ્વારા આપવામાં આવેલા પરિણામો, જોખમ સ્કોર અને વિશ્લેષણ માત્ર અંદાજિત છે. તેને આખરી કાનૂની, નાણાકીય કે સાયબર સુરક્ષા ખાતરી ન માનવી.'
                  : 'DeTexSO is an AI-powered educational and awareness platform. Analysis results, risk scores, and classifications generated by Artificial Intelligence are probabilistic estimates and must NOT be treated as absolute guarantees or legal/financial advice.'}
              </p>
            </div>
            ...
            <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700/60 rounded-2xl space-y-1">
              ...
              <p className="text-slate-600 dark:text-slate-300 text-[11px] sm:text-xs leading-relaxed">
                {language === 'hi'
                  ? 'डेवलपर्स, निर्माता और ऑपरेटर इस प्लेटफ़ॉर्म के उपयोग या इस पर की गई कार्रवाई से होने वाले किसी भी प्रत्यक्ष या अप्रत्यक्ष नुकसान के लिए उत्तरदायी नहीं होंगे। DeTexSO एक स्वतंत्र प्लेटफॉर्म है और किसी भी बैंक या सरकारी संस्था से संबद्ध नहीं है।'
                  : language === 'gu'
                  ? 'ડેવલપર્સ, ઉત્પાદકો અને ઓપરેટરો આ પ્લેટફોર્મના ઉપયોગથી કે તેના આધારે લીધેલા પગલાંથી થતા કોઈપણ નુકસાન માટે જવાબદાર રહેશે નહીં. DeTexSO એક સ્વતંત્ર પ્લેટફોર્મ છે અને કોઈ બેંક કે સરકારી સંસ્થા સાથે જોડાયેલ નથી.'
                  : 'The developers, creators, and platform operators shall not be held liable for any direct, indirect, financial loss, or consequences arising from the use of or reliance upon this application. DeTexSO is an independent hackathon MVP and is not affiliated with any official banking or government institution.'}
              </p>
            </div>
